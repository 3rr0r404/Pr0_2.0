...
(function() {
    ...
    function waitForPage(){
        if(typeof p !== "undefined"){
            ...
            p.View.Stream.Main.prototype.showItem = function($item, scrollTo) {
                ...
                var previousItemHeight;
                if (this.$itemContainer) {
                    previousItemHeight = this.$itemContainer.find('.item-image').height() || 0;
                }
            ...
            p.View.Stream.Main.prototype.loaded = function(items, position, error) {
                ...
                if (position == p.Stream.POSITION.PREPEND) {
                    ...
                    html = this.buildItemRows(items, 0, items.length - numPlaceholders, position);
                    ...
                    if (itemCount % this.itemsPerRow != 0) {
                        ...
                        for (var j = 0; j < fill; j++) {
                            html += this.buildItem(items[j]);
                        }
                        lastRow.append(this.prepareThumbsForInsertion(html));
                    }
                    html = this.buildItemRows(items, fill, items.length, position);
    ...
    var css = '#upload-form input[type="submit"] { position:relative; top: 420px; left: 350px; }'+
        ...
        '.item-comments { height: calc(100vh - 51px) !important; border-right: 3px solid rgb(42, 46, 49); background: none repeat scroll 0% 0% rgba(23, 23, 24, 0.89); overflow-x:hidden; top: 51px !important; width: 400px !important;}' +//'+high+'
        '.item-container-content { overflow: visible !important; vertical-align: middle; height: calc(100% - 52px) !important; width: calc(100% - 402px) !important;  left: 400px;}'+
        ...
        '#footer-links { background-color: #000000; line-height: 1.6 !important; text-align: center !important; top: 7px; left: auto !important; right: 340px !important; height: 20px; width: 160px !important; bottom: 0px !important; margin: 0 !important}'+
        ...
        if ($('#stream').length) {
            ...
            open.onsuccess = function (evt) {
                ...
                if (window.location.pathname.match('/(top)')) {
                    ...
                    first = $('.stream-row a:first').attr('id');
                }
    ...
    function imagechange() {
        ...
        var itemname = 'item-' + itemId.substring(itemId.length - 7, itemId.length);
        var posi = document.getElementById(itemname).offsetTop - 52;
    ...
    function initiateZoom() {
        ...
        if($('.item-image').attr('type')=="video/mp4"){ } else {
            $('.item-image').hide();
            $('.item-info').hide();
        }
        ratioCalculation();
    ...
    function getPageResolution(){
        ...
        if($('.item-image').attr('type')=="video/mp4"){
            console.log("video/mp4 detected");
    ...
    function zooming(itemNaturalHeight,itemNaturalWidth,content){
        ...
        if($('.item-image').attr('type')=="video/mp4"){
            $(".video-controls").css( 'width', $('.item-image').width());
            $(".video-controls").show();
        }
        $('.item-image').show();
        $('.item-info').show();
